...
1. Invoke skills under `.claude/skills/` (or `.valid/skills/`) **manually**: `/plan`, `/spec`, `/build-feature`, `/audit`, `/finish`, `/patch`, `/minipatch`, `/delegate`.
...
4. Edit `.valid/features/<slug>/data.json` yourself. CLI is scaffolding (`valid feature`, `gate`, `doctor`, `paths`, `promote`, `merge`, `env`).
5. MCP server `valid` is knowledge RAG + project scripts under `.valid/scripts/`.
6. **Path contract:** stay on the principal folder; write product code under `CODE_ROOT` from `valid paths <slug>`.
