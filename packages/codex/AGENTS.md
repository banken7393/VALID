...
3. **Edit** `.valid/features/<slug>/data.json`, Mermaid, and workspace files yourself. The `valid` CLI is minimal scaffolding and plumbing (`feature`, `gate`, `doctor`, `paths`, `promote`, `merge`, `env`, optional `board`/`task`/`report` helpers).
...
5. **Path contract:** stay on the principal folder; write product code under `CODE_ROOT` from `valid paths <slug>` (`.valid/worktrees/<slug>/`).
